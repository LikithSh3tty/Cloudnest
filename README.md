...
**Reading the numbers straight: this run cannot distinguish fine-tuning's effect from noise on any headline metric.** At n=54, one question is worth ~1.85 percentage points, and the binomial standard error near p≈0.65 is ~6.5pp — conventional 95% confidence would need a delta of roughly ±13pp. Nothing above gets close: recall@1 moved -1.9pp, recall@3 moved +3.7pp, MRR@10 moved +0.4pp. The honest description is that the result is directionally consistent with a shift from top-1 precision toward top-3 coverage, but not statistically distinguishable from no effect at this sample size. Per-kind, `lookup` recall@1 went 0.811 → 0.784 while `multi` recall@1 sat flat at 0.353 and `multi` recall@3 went 0.882 → 1.000 (MRR@10 0.602 → 0.647) — a lookup-down/multi-up split that's worth noting as suggestive, since noise wouldn't obviously correlate with question kind, but it is not established by this sample size either.
