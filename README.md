...
**Reading the numbers straight: this run cannot distinguish fine-tuning's effect from noise on any headline metric.** At n=54, one question is worth ~1.85 percentage points. These are paired comparisons — baseline and fine-tuned scored on the same 54 questions — so the right instrument is McNemar's test on discordant pairs, not a single-proportion bar applied blanket across metrics; the binomial standard error itself varies by metric too: ~6.4pp near recall@1's p≈0.667, ~3.6pp near recall@3's p≈0.926. Nothing above gets close: recall@1 moved -1.9pp, recall@3 moved +3.7pp, MRR@10 moved +0.4pp — in different directions across the three metrics, which is exactly the pattern noise alone produces, not evidence of a precision-to-coverage trade-off. recall@3's +3.7pp sits inside roughly ±7pp (2×SE) on its own, and it's carried by just 2 discordant pairs — McNemar's test would need about 6 for significance at conventional 95% confidence, so the null survives the more appropriate paired test too, not only the looser one. Per-kind, `lookup` recall@1 went 0.811 → 0.784 while `multi` recall@1 sat flat at 0.353 and `multi` recall@3 went 0.882 → 1.000 (MRR@10 0.602 → 0.647); that split is curious, but this experiment cannot resolve whether it means anything — testing it would need a larger eval set, and at n=54 it is not a finding.
...
4. **Four eval questions have near-duplicate counterparts in the training set, not two.** Same alnum-normalized `difflib` ratio, restricted to pairs sharing a gold section, finds four at ≥0.70: eval *"How much does the Pro plan cost?"* vs. training *"how much does the paid version cost"* (0.818, *Plans and Pricing*) is the worst case; then *"How do I permanently delete my account?"* vs. *"how do i delete my account"* (0.812, *Deleting an Account*); *"How can I recover deleted files?"* vs. *"can i get deleted files back"* (0.712, *Files Missing After Sync*); and *"Does CloudNest support two-factor authentication?"* vs. *"does cloudnest support totp apps"* (0.709, *Two-Factor Authentication (2FA)*). The exact-match leakage gate (`backend/tests/test_eval_harness.py`) reads 0, but scores on these four items may be mildly optimistic.
