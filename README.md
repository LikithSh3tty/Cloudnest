...
That embedding model has also been fine-tuned on this corpus, and the result was measured rather than assumed. Against 54 hand-labeled questions the model never trained on, the tuned version did not beat the frozen one by anything the sample size can distinguish from noise, so the frozen model is still what ships. Both are in the tree behind one environment variable. [Fine-tuning the retriever](#fine-tuning-the-retriever) has the numbers, the method, and the limitations.
...
- **Bails out honestly** when it isn't confident. Below a threshold calibrated against real questions (see `backend/calibrate.py`), it asks you to add detail rather than guessing. Each embedding model gets its own gate, since a different model puts the cosine scores on a different scale: 0.30 for the frozen model, 0.28 for the fine-tuned one.
- **Knows how good its retrieval actually is.** 54 hand-labeled questions measure whether the right section comes back, not just whether the score looks confident: recall@1 0.667, recall@3 0.926, MRR@10 0.788. The same harness shows where it's weak, which is questions spanning several sections at once (recall@1 0.353 against 0.811 for single-fact lookups).
...
│   ├── variant.py           # EMBED_VARIANT switch; pairs model + index atomically
│   ├── build_index.py       # offline indexer; embeds cloudnest_docs/ into index.npz
│   ├── calibrate.py         # measures the confidence threshold from real questions, per variant
│   ├── index.npz            # committed embedding index (vectors + chunk metadata) — baseline
│   ├── index_finetuned.npz  # same, built from the fine-tuned model (EMBED_VARIANT=finetuned)
│   ├── model/                # vendored int8 embedding models + tokenizer
│   │   ├── model_quint8_avx2.onnx           # frozen baseline (default)
│   │   ├── model_finetuned_quint8_avx2.onnx # fine-tuned on finetune/data/train_pairs.jsonl
│   │   └── tokenizer.json
│   ├── tests/                # pytest suite (96 tests, incl. tests/test_variant.py)
│   ├── requirements.txt
...
│   └── 07_general_faq.md
├── finetune/                # fine-tuning experiment: train, export, benchmark (see
│   │                        # "Fine-tuning the retriever" below); does not touch the
│   │                        # live backend directly
│   ├── make_pairs.py        # generates synthetic (question, section) training pairs
│   ├── train.py             # fine-tunes all-MiniLM-L6-v2 with MultipleNegativesRankingLoss
│   ├── export_onnx.py       # exports the fp32 checkpoint to int8 ONNX for serving
│   ├── eval_retrieval.py    # the measuring instrument: recall@1/@3, MRR@10 per variant
│   ├── data/
│   │   ├── train_pairs.jsonl     # 464 synthetic training pairs (do not modify; see README)
│   │   └── eval_questions.jsonl  # 54 hand-labeled held-out questions, never trained on
│   ├── artifacts/            # fp32 checkpoint + training log (not committed as final weights)
│   └── README.md
├── frontend/
│   ├── src/
│   │   ├── App.jsx          # the chat UI
...
The retriever above runs `all-MiniLM-L6-v2` exactly as published — frozen, never trained on this corpus. The obvious next question is whether adapting it to CloudNest's own docs and question phrasing beats that frozen baseline by enough to justify swapping it in. `finetune/` answers that question end to end, and the result is why `backend/variant.py` still defaults to `baseline`.

**What was tuned, and how.** `sentence-transformers/all-MiniLM-L6-v2` was fine-tuned with `MultipleNegativesRankingLoss` — in-batch negatives, `BatchSamplers.NO_DUPLICATES` so no batch ever pairs two rows from the same section as each other's accidental negative — on 464 (question, section) pairs spanning 40 of the corpus's 46 sections, for 3 epochs at 15 batches per epoch (`ceil(464/32)`), batch size 32. The tuned checkpoint was exported to the same int8 ONNX format the frozen model already ships in (`finetune/export_onnx.py`), so it drops into the exact runtime path `backend/embed.py` already has, just under a different filename.

**The training pairs are synthetic — plainly, not a valid eval set.** All 464 pairs were generated by having Claude read each doc section and write plausible user questions against it (`finetune/make_pairs.py`), not collected from real users. A model scored against its own training generator's phrasing can look artificially strong, since it's partly matching the generator's voice rather than a real one. That's why this set is used only to train, never to measure.

**The held-out eval set is the only honest measurement.** `finetune/data/eval_questions.jsonl` holds 54 questions, hand-written and hand-labeled against the corpus independently of the training-pair generator, each `(doc, title)` label checked against `backend.app.load_chunks()` output to confirm the section actually answers the question. None of it was used for training.

| Variant | recall@1 | recall@3 | MRR@10 |
|---|---|---|---|
| baseline (frozen int8) | 0.667 | 0.926 | 0.788 |
| fine-tuned (fp32) | 0.648 | 0.963 | 0.793 |
| fine-tuned (int8, served) | 0.648 | 0.963 | 0.792 |

Before reading these as a comparison at all: every one of the 30 distinct gold sections the 54 eval questions target already has training pairs, so there is no untrained control slice here to check for regression against — see the first limitation below for the full explanation.

**Reading the numbers straight: this run cannot distinguish fine-tuning's effect from noise on any headline metric.** At n=54, one question is worth ~1.85 percentage points. These are paired comparisons — baseline and fine-tuned scored on the same 54 questions — so the right instrument is McNemar's test on discordant pairs, not a single-proportion bar applied blanket across metrics; the binomial standard error itself varies by metric too: ~6.4pp near recall@1's p≈0.667, ~3.6pp near recall@3's p≈0.926. Nothing above gets close: recall@1 moved -1.9pp, recall@3 moved +3.7pp, MRR@10 moved +0.4pp — in different directions across the three metrics, which is exactly the pattern noise alone produces, not evidence of a precision-to-coverage trade-off. recall@3's +3.7pp sits inside roughly ±7pp (2×SE) on its own, and its net +2 comes from 4 discordant pairs, 1 favoring baseline and 3 favoring fine-tuned — exact McNemar on that 1/3 split gives p≈0.625, nowhere near the conventional 0.05 bar, so the null survives the more appropriate paired test too, not only the looser one. Per-kind, `lookup` recall@1 went 0.811 → 0.784 while `multi` recall@1 sat flat at 0.353 and `multi` recall@3 went 0.882 → 1.000 (MRR@10 0.602 → 0.647); that split is curious, but this experiment cannot resolve whether it means anything — testing it would need a larger eval set, and at n=54 it is not a finding.

**Limitations, stated up front rather than buried:**

1. **The control slice is empty.** All 30 distinct gold sections the 54 eval questions target have training pairs — train and eval are disjoint in question *text*, not in target *section*. So this run cannot separate "fine-tuning helped the sections it trained on" from "fine-tuning helped generally"; there is no untrained slice to check for regression against. This is the most important caveat here.
2. **Training data is synthetic and circular by construction** (see above) — the hand-labeled held-out set is the only number in this section that means what it looks like it means.
3. **The corpus is small.** 46 sections across 7 documents, 464 training pairs across 40 sections; the other 6 are bare-H1 title chunks with no answerable content to train on. A small corpus leaves little for domain adaptation to exploit, which is a plausible explanation for a null result on top of the noise floor above.
4. **Five (eval, training) pairs land above a 0.70 near-duplicate ratio, across four distinct eval questions.** Same alnum-normalized `difflib` ratio, restricted to pairs sharing a gold section: eval *"How much does the Pro plan cost?"* vs. training *"how much does the paid version cost"* (0.818, *Plans and Pricing*) is the worst case; the same eval question also matches training *"how much does more space cost"* (0.767, *Plans and Pricing*); then *"How do I permanently delete my account?"* vs. *"how do i delete my account"* (0.812, *Deleting an Account*); *"How can I recover deleted files?"* vs. *"can i get deleted files back"* (0.712, *Files Missing After Sync*); and *"Does CloudNest support two-factor authentication?"* vs. *"does cloudnest support totp apps"* (0.709, *Two-Factor Authentication (2FA)*). The exact-match leakage gate (`backend/tests/test_eval_harness.py`) reads 0, but scores on these four eval items may be mildly optimistic.
5. **`ESCALATE_FLOOR` (0.27) was not recalibrated for the tuned variant.** Its borderline "ask for clarification" band is 0.27–0.28 under `finetuned` versus 0.27–0.30 under `baseline` — not a correctness bug (the finetuned in-scope floor sits well above at 0.365), but a third of the band width, and worth revisiting if the tuned variant is ever made default.

**`baseline` remains the default.** The rule was fixed in writing before Task 5 produced a single number: the tuned variant becomes default only if it beats baseline on recall@1 *and* does not regress recall@3. recall@1 regressed (0.667 → 0.648), so the rule says no, and it was applied as written rather than renegotiated after seeing the result — that's the point of registering it first. The tuned artifacts stay in the tree and stay reachable.

Set `EMBED_VARIANT=finetuned` to run on the tuned model and index instead of the frozen baseline; unset, empty, or any unrecognized value resolves to `baseline` (`backend/variant.py`). Reverting is the same switch in reverse — unset the variable, or set it to `baseline` explicitly — no rebuild required, since both models and both indexes are already committed and both are covered by `backend/tests/test_variant.py`.
...
- Frontend tests. The backend has 96 pytest cases around the router, the parallel retrievers, fusion, the gate, the index, the ticket store, and the API's auth gate; the React side, chat and admin both, is only checked by hand.
- Stop indexing the 6 bare-H1 title chunks. They carry no answerable content but still sit in the retrieval index competing on cosine similarity against real sections. `load_chunks()` was deliberately left unchanged for the fine-tuning experiment (see [Fine-tuning the retriever](#fine-tuning-the-retriever)) because touching chunking would have invalidated the immutable baseline `index.npz` this whole before/after comparison depends on — but with that comparison now recorded, dropping them is a clean, low-risk improvement.
- Expand the held-out eval set with questions targeting those same 6 sections once they're either enriched with content or excluded, so a future retrieval change has a real control slice to check for regressions against — this run's eval set had none (see the fine-tuning limitations above).
