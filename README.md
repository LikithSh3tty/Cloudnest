...
│   │   └── tokenizer.json
│   ├── tests/                # pytest suite (93 tests, incl. tests/test_variant.py)
│   ├── requirements.txt
...
**Reading the numbers straight: this run cannot distinguish fine-tuning's effect from noise on any headline metric.** At n=54, one question is worth ~1.85 percentage points, and the binomial standard error near p≈0.65 is ~6.5pp — conventional 95% confidence would need a delta of roughly ±13pp. Nothing above gets close: recall@1 moved -1.9pp, recall@3 moved +3.7pp, MRR@10 moved +0.4pp — in different directions across the three metrics, which is exactly the pattern noise alone produces, not evidence of a precision-to-coverage trade-off. Per-kind, `lookup` recall@1 went 0.811 → 0.784 while `multi` recall@1 sat flat at 0.353 and `multi` recall@3 went 0.882 → 1.000 (MRR@10 0.602 → 0.647); that split is curious, but this experiment cannot resolve whether it means anything — testing it would need a larger eval set, and at n=54 it is not a finding.
...
4. **Two eval questions have near-duplicate counterparts in the training set** (fuzzy-match ratio 0.812 and 0.712). The exact-match leakage gate reads 0, but scores on those two items may be mildly optimistic.
...
- Frontend tests. The backend has 93 pytest cases around the router, the parallel retrievers, fusion, the gate, the index, the ticket store, and the API's auth gate; the React side, chat and admin both, is only checked by hand.
